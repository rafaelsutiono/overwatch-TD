...
public class TileManager {
	...
	private void createTiles() { // images of tiles and their rotated counterparts

		int id = 0;

		tiles.add(GRASS = new Tile(getSprite(9, 0), id++, GRASS_TILE)); // id 1
		tiles.add(WATER = new Tile(getAniSprites(0, 0), id++, WATER_TILE)); // id 2

		roadsS.add(ROAD_LR = new Tile(getSprite(8, 0), id++, ROAD_TILE)); // and so and so
	...
	private void loadAtlas() {
		atlas = LevelMaker.getSpriteAtlas();
	}
	...
	private BufferedImage[] getAniSprites(int xCord, int yCord) { // stages of water tile animation
	...
	public boolean isSpriteAnimation(int spriteID) { // check if sprite is an animated one
		return tiles.get(spriteID).isAnimation();
	}

	public int[][] getTypeArr() { // retrieve tile types according to ID using a nested loop to iterate over each x & y
		int[][] idArr = LevelMaker.GetLevelData(); // 2d array
		int[][] typeArr = new int[idArr.length][idArr[0].length]; // 2d array to store tile types
