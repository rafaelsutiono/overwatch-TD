...
public class Toolbar extends Bar {
	...
	private void initPathImgs() {
		pathStart = LevelMaker.getSpriteAtlas().getSubimage(7 * 32, 2 * 32, 32, 32);
		pathEnd = LevelMaker.getSpriteAtlas().getSubimage(8 * 32, 2 * 32, 32, 32);
	}
	...
	public void draw(Graphics g) {
		...
		g.drawString("Press 'R' to rotate interactive tiles.", 340, 785);
	}

	private void drawButtons(Graphics g) { // method to draw buttons
		bMenu.draw(g);
		bSave.draw(g);
	...
	private void drawPathButton(Graphics g, MyButton b, BufferedImage img) { // start & endpoint buttons
	...
	private void drawNormalButton(Graphics g, MyButton b) { // menu & save buttons
		g.drawImage(getButtImg(b.getId()), b.x, b.y, b.width, b.height, null);
		drawButtonFeedback(g, b);

	}

	private void drawMapButtons(Graphics g) { // tile buttons
	...
	private void drawSelectedTile(Graphics g) { // draw selected tile on the right side
	...
	public void mouseClicked(int x, int y) { // mouseclick actions
	...
	public void mouseMoved(int x, int y) { // mousemoved actions
	...
	public void mousePressed(int x, int y) { // mousepressed actions
		if (bMenu.getBounds().contains(x, y))
	...
	public void mouseReleased(int x, int y) { // reset setMousePressed() to false
		bMenu.resetBooleans();
		bSave.resetBooleans();
