...
import java.awt.Graphics2D; // for rotating images
...
public class ImgFix {
	...
	public static BufferedImage getRotImg(BufferedImage img, int rotAngle) {
		...
		g2d.rotate(Math.toRadians(rotAngle), w / 2, h / 2); // rotate img on its center -> w/2 and h/2
	...
	// rotate 2nd img only (top layer img)
	public static BufferedImage getBuildRotImg(BufferedImage[] imgs, int rotAngle, int rotAtIndex) {
		...
		for (int i = 0; i < imgs.length; i++) { // do a rotation for every iteration
			if (rotAtIndex == i)
	...
	// rotate 2nd img only (bottom layer has animation)
	public static BufferedImage[] getBuildRotImg(BufferedImage[] imgs, BufferedImage secondImage, int rotAngle) {
		...
		for (int i = 0; i < imgs.length; i++) {
			...
			// sequence: draw water tile -> rotate 2nd img -> draw new img -> dispose of the previous one -> put new img in the array
			g2d.drawImage(imgs[i], 0, 0, null);
			g2d.rotate(Math.toRadians(rotAngle), w / 2, h / 2);
